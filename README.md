...
The root `requirements.txt` contains the complete optional Python environment for YOLOv5 training, evaluation and ONNX export. It does not replace the Java and Node.js prerequisites because pip cannot install those system runtimes.
...
│   └── styles.css
├── package.json
├── requirements.txt           # Optional YOLO training and ONNX export tools
└── README.md
```
...
### Install optional YOLO training and export tools

The application itself does not require Python. Run this section only when you want to train, evaluate or export a YOLO model:
...
For an NVIDIA GPU, use the command generated by the official [PyTorch installation selector](https://pytorch.org/get-started/locally/) if its CUDA-specific build differs from the default pip package.
